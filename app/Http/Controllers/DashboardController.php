...
class DashboardController extends Controller
{
    ...
    public function gantiWarna(Request $request)
    {
        Rak::where('id', $request->id)->update([
            'warna' => $request->warna
        ]);

        return redirect()->back()->with('success', 'Data warna berhasil diubah');
    }

    public function printDataLayout()
    {
        return view('dashboard.print_data_layout', [
            'block' => Block::where('gudang_id', Session::get('gudang_id'))->with(['cell', 'cell.rak'])->get(),
        ]);
    }
}
