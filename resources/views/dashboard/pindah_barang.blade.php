<div class="row">
    ...
    <div class="mt-2 col-6 col-md-3">
        <div class="form-group">
            ...
            <select name="block_id" class="form-control" id="block" onchange="getCell(this);" required>
                @foreach ($block as $b)
                    <option value="{{ $b->id }}" {{ $stok->block_id == $b->id ? 'selected' : '' }}>
                        {{ $b->nm_block }}</option>
                @endforeach
            </select>
        </div>
    ...
    <div class="mt-2 col-6 col-md-3">
        <div class="form-group">
            ...
            <select name="cell_id" class="form-control" id="cell" onchange="getRak(this);" required>
                @foreach ($cell as $b)
                    <option value="{{ $b->id }}" {{ $stok->cell_id == $b->id ? 'selected' : '' }}>
                        {{ $b->nm_cell }}</option>
                @endforeach
            </select>
        </div>
    ...
    <div class="mt-2 col-6 col-md-3">
        <div class="form-group">
            ...
            <select name="rak_id" class="form-control" id="rak" onchange="getPallet(this);" required>
                @foreach ($rak as $b)
                    <option value="{{ $b->id }}" {{ $stok->rak_id == $b->id ? 'selected' : '' }}>
                        {{ $b->nm_rak }}</option>
                @endforeach
            </select>
        </div>
    ...
    <div class="col-4 mt-2">
        <div class="form-group">
            <label for="">Jumlah
                {{ Session::get('gudang_id') == 3 || Session::get('gudang_id') == 4 ? 'Karung' : 'Box' }}</label>
            <input type="number" name="debit_box" class="form-control" value="{{ $stok->sisa_box }}"
                max="{{ $stok->debit_box }}" required>
        </div>
    </div>
    ...
    <div class="mt-2 col-4">
        <div class="form-group">
            ...
            <input type="number" name="debit_pak" class="form-control" value="{{ $stok->sisa_pak }}"
                max="{{ $stok->debit_pak }}" required>
        </div>
    </div>
    ...
    <div class="mt-2 col-4">
        <div class="form-group">
            ...
            <input type="number" name="debit_kg" class="form-control" value="{{ $stok->sisa_kg }}"
                max="{{ $stok->debit_kg }}" required>
        </div>
    </div>
