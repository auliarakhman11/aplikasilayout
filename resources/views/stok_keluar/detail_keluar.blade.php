                ...
                <form action="{{ route('addCheckerKeluar') }}" method="post">
                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            {{-- <a href="{{ route('pdfDetailKeluar', $checker[0]->kd_gabungan) }}" target="_blank"
                                    ...
                                    <tbody class="table-border-bottom-0">
                                        @foreach ($checker as $index => $c)
                                            <tr>
                                                ...
                                                <td>{{ $c->block->nm_block }}<br>{{ $c->cell->nm_cell }}<br>{{ $c->rak->nm_rak }}<br>Pallet
                                                    {{ $c->pallet_id }}</td>
                                                <td>{{ $c->kredit_box }}
                                                    {{ Session::get('gudang_id') == 3 || Session::get('gudang_id') == 4 ? 'Karung' : 'Box' }}<br>{{ $c->kredit_pak }}
                                                    Pak<br>{{ $c->kredit_kg }} KG</td>
                                                <td>
                                                    <div class="form-check form-switch mb-2">
                                                        <input class="form-check-input terima" name="terima[]"
                                                            checker_id="{{ $c->id }}" value="{{ $c->id }}"
                                                            type="checkbox">
                                                    </div>
                                                </td>
                                                <td>
                                                    <textarea class="form-control" rows="3" cols="5" name="ket_checker[]" id="ket_{{ $c->id }}"
                                                        required></textarea>
                                                    <input type="hidden" id="tolak_{{ $c->id }}" name="tolak[]"
                                                        value="{{ $c->id }}">
                                                </td>
                                            </tr>
                                        @endforeach
