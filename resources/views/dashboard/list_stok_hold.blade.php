...
@section('content')
                            ...
                            <div class="row">
                                ...
                                <div class="col-5 col-md-3">
                                    <div class="form-group">
                                        ...
                                        <input type="date" class="form-control" value="{{ $tgl1 }}"
                                            name="tgl1" required>
                                    </div>
                                </div>
                                <div class="col-5 col-md-3">
                                    <div class="form-group">
                                        ...
                                        <input type="date" class="form-control" value="{{ $tgl2 }}"
                                            name="tgl2" required>
                                    </div>
                                </div>

                                <div class="col-2 col-md-1">
                                    <button class="btn btn-sm btn-primary mt-4" type="submit"><i
                                ...
                                <tbody>
                                    @foreach ($stok as $index => $s)
                                        <tr>
                                            ...
                                            <td>{{ $s->block->nm_block }}<br>{{ $s->cell->nm_cell }}<br>{{ $s->rak->nm_rak }}<br>Pallet
                                                {{ $s->pallet_id }}</td>
                                            <td>{{ $s->debit_box }}
                                                {{ Session::get('gudang_id') == 3 || Session::get('gudang_id') == 4 ? 'Karung' : 'Box' }}<br>{{ $s->debit_pak }}
                                                Pack<br>{{ $s->debit_kg }} Kg</td>
                                            <td>{{ $s->shift->nm_shift }}</td>
                                            <td>{{ $s->user->name }}</td>
                                            <td>
                                                <button data-bs-toggle="modal" data-bs-target="#modal_edit_stok"
                                                    stok_id="{{ $s->id }}"
                                                    class="btn btn-primary btn-sm btn_edit_stok"><i
                                                        class='bx bxs-edit'></i></button>
                                                <a href="{{ route('deleteStokHold', $s->id) }}"
    ...
    <form method="POST" action="{{ route('editStokMasuk') }}">
        @csrf
        <div class="modal fade" id="modal_edit_stok" tabindex="-1" aria-labelledby="modal_edit_stokLabel"
            aria-hidden="true">
