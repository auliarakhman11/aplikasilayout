...
<body>
    ...
    <div class="container-fluid">
        ...
        <div class="container">
            ...
            <center>
                <h5>FORM CHECKER BARANG MASUK</h5>
            </center>
        </div>
                ...
                <tbody>
                    @foreach ($stok as $d)
                        <tr>
                            ...
                            <td style="text-align: center;">{{ $d->debit_box }}
                                {{ Session::get('gudang_id') == 3 || Session::get('gudang_id') == 4 ? 'Karung' : 'Box' }}<br>{{ $d->debit_pak }}
                                Pack<br>{{ $d->debit_kg }} Kg</td>
                            <td style="text-align: center;">{{ $d->block->nm_block }} <br>{{ $d->cell->nm_cell }}
                                <br>{{ $d->rak->nm_rak }} <br>Pallet {{ $d->pallet_id }}
                            </td>
                            <td></td>
                        </tr>
                    @endforeach
