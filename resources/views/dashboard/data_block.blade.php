...
@section('content')
            ...
            <div class="col-12 mb-4 order-0">
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <a href="{{ route('printDataLayout') }}" target="_blank" class="btn btn-primary btn-sm float-end"><i
                        ...
                        <div class="scroll_horizontal">
                            ...
                            @foreach ($block as $b)
                                <table border="1px solid" style="font-size: 20px;">
                                    <thead>
                                        <tr>
                                            <th style="background-color: tomato"><b><a href="#modal_detail_block"
                                                        data-bs-toggle="modal" style="text-decoration: none; color: black;"
                                                        class="btn_detail_block"
                                                        block_id="{{ $b->id }}">{{ $b->nm_block }}</a></b></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($b->cell as $c)
                                            <tr>
                                                <td style="background-color: skyblue"><b><a href="#modal_detail_cell"
                                                            data-bs-toggle="modal"
                                                            style="text-decoration: none; color: black;"
                                                            class="btn_detail_cell"
                                                            cell_id="{{ $c->id }}">{{ $c->nm_cell }}</a></b>
                                                    {{-- <a target="_black" href="{{ route('generateQr', $c->id) }}"
                                                        class="btn btn-xs btn-primary"><i class='bx bx-qr'></i></a> --}}
                                                </td>
                                            </tr>
                                            @foreach ($c->rak as $r)
                                                <tr style="background-color: {{ $r->warna }}">
                                                    <td><a href="#modal_detail_rak" data-bs-toggle="modal"
                                                            style="text-decoration: none; color: black;"
                                                            class="btn_detail_rak"
                                                            rak_id="{{ $r->id }}">{{ $r->nm_rak }}</a> <a
                                                            href="#modal_warna_lantai{{ $r->id }}"
                                                            data-bs-toggle="modal"><i class='bx bx-palette'></i></a></td>
                                                </tr>
                                            @endforeach
                                        @endforeach
    ...
    @foreach ($block as $b)
        @foreach ($b->cell as $c)
            @foreach ($c->rak as $r)
                <form action="{{ route('gantiWarna') }}" method="post">
                    @csrf
                    <div class="modal fade" id="modal_warna_lantai{{ $r->id }}" tabindex="-1"
                        aria-labelledby="modal_warna_lantai{{ $r->id }}Label" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered ">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="modal_warna_lantai{{ $r->id }}Label">Ganti Warna
                                        {{ $r->nm_rak }}</h5>
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input class="form-control" type="color"
                                            value="{{ $r->warna ? $r->warna : '#FFFFFF' }}" name="warna">
                                    </div>
                                </div>
    ...
    <div class="modal fade" id="modal_detail_block" tabindex="-1" aria-labelledby="modal_detail_blockLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered ">
            <div class="modal-content">
                <div class="modal-header">
    ...
    <div class="modal fade" id="modal_detail_cell" tabindex="-1" aria-labelledby="modal_detail_cellLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered ">
            <div class="modal-content">
                <div class="modal-header">
    ...
    <div class="modal fade" id="modal_detail_rak" tabindex="-1" aria-labelledby="modal_detail_rakLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered ">
            <div class="modal-content">
                <div class="modal-header">
    ...
    <form id="form_pindah_barang">
        <div class="modal fade" id="modal_pindah_barang" tabindex="-1" aria-labelledby="modal_pindah_barangLabel"
            aria-hidden="true">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.btn_get_pindah_barang', function() {
                ...
                $("#table_pindah_barang").html(
                    '<div class="spinner-border text-primary" role="status"><span class="visually-hidden">Loading...</span></div>'
                );

                $.get('getPindahBarang/' + pallet_id + '/' + tgl_exp + '/' + barang_id + '/' + block_id +
                    '/' + cell_id + '/' + rak_id,
                    function(data) {
